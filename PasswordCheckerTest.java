...
public class PasswordCheckerTest {
	...
	/**
	 * Create student passwords arraylist
	 * @author Auguste K.
	 */
	...
	public void setUp() throws Exception {
		...
		*/
		/**
		 * Arraylist of String to use for the testValidPasswordsSTUDENT test
		 * @author Auguste K.
		 */
		String[] d = {"229911PP", "Ua9#zaal8V", "franck6AAA", "9tim#", "keyane66#", "3ckyaApril#", 
				"basile02", "ijklmn", "Bananapp#", "uu22j", "laborPractic", "myUsername", 
				"myUsername2"};
		passwordsStudent = new ArrayList<String>();
		passwordsStudent.addAll(Arrays.asList(d)); // puts strings into the ArrayList
	}
	...
	public void testGetInvalidPasswordsSTUDENT() {
		...
		ArrayList<String> results;
		results = PasswordCheckerUtility.getInvalidPasswords(passwordsStudent);
		Scanner scan = new Scanner(results.get(0)); //
		assertEquals(scan.next(), "229911PP");
		String nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("lowercase"));
		//assertEquals(scan.nextLine(), " The password must contain at least one lowercase alphabetic character.");
		scan = new Scanner(results.get(1)); //
		assertEquals(scan.next(), "franck6AAA");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("special"));
		//assertEquals(scan.nextLine(), " The password cannot contain more than two of the same character in sequence.");
		scan = new Scanner(results.get(3)); //
		assertEquals(scan.next(), "keyane66#");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("uppercase"));
		//assertEquals(scan.nextLine(), " The password must contain at least one uppercase alphabetic character.");
		scan = new Scanner(results.get(5)); //
		assertEquals(scan.next(), "ijklmn");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("uppercase") || nextResults.contains("digit"));
		//assertEquals(scan.nextLine(), " The password must contain at least one digit.");
		scan = new Scanner(results.get(6)); //a
		assertEquals(scan.next(), "Bananapp#");
		nextResults = scan.nextLine().toLowerCase();
		assertTrue(nextResults.contains("digit"));
		//assertEquals(scan.nextLine(), " The password must contain at least one digit.");
	}
